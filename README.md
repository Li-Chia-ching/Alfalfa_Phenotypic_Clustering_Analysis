...
## 📖 Scientific Rationale / 科学原理

**Why use multivariate clustering?**
In agricultural science, multivariate phenotypic clustering is a standard method for germplasm evaluation.
* **Scientific Consensus**: Guidelines from the International Plant Genetic Resources Institute (IPGRI) recommend using multiple morphological indicators (e.g., height, leaf shape, branching) simultaneously for clustering to capture overall germplasm diversity.
* **Methodological Validity**: This script follows standard biometric procedures:
    1.  **Data Aggregation**: Averaging multiple samples per line to reduce individual variance.
    2.  **Standardization**: Using Z-score normalization (`scale()`) to handle unit differences between traits (e.g., cm vs. count), preventing one trait from dominating the distance calculation.
    3.  **Distance Calculation**: Computing Euclidean distances to quantify phenotypic dissimilarity.
    4.  **Clustering**: Utilizing Ward's minimum variance method to create compact, spherical clusters.
* **Software Alignment**: The logic mirrors established software like NTSYS-pc and the R `cluster` package, ensuring results are comparable with peer-reviewed literature.

**为什么使用多变量聚类？**
在农业科学中，多表型聚类广泛用于种质资源评价。
* **科学共识**：国际植物遗传资源研究所（IPGRI）的指南推荐使用多个形态指标（如高度、叶形、分枝）一起聚类，以全面捕捉种质多样性。
* **方法有效性**：本脚本遵循标准的生物统计流程：
    1.  **数据聚合**：计算每个株系的平均值以减少个体差异。
    2.  **标准化**：使用 Z-score 标准化（`scale()`）处理不同性状间的单位差异（如厘米与个数），防止单一性状主导距离计算。
    3.  **距离计算**：计算欧氏距离以量化表型差异。
    4.  **聚类**：采用 Ward 最小方差法生成紧凑的聚类簇。
* **软件对标**：该逻辑与 NTSYS-pc 和 R `cluster` 包等主流软件一致，确保结果具有可比性。
